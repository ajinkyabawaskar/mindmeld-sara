...
def send_accomodation(request, responder):
    ...
    for e in request.entities:
        if(e['type']=='location'):
            origin = e['text']
    if(origin != ""):
        hotels = _get_poi_from_name(origin=origin, poi='hotels')
        if(hotels != False):
            responder.slots['hotels'] = "\n".join(hotels)
            responder.reply('Hotels in: '+origin.title()+'{hotels}')
        else:
            responder.reply('I am having trouble connecting with my hotel guy D: Please try again later.')
    else:
        responder.reply("Oops! I am not aware of that location yet! :(")            
...
def confirm_head_count(request,responder):
    for entity in request.entities:
        if(entity['type'] == 'sys_number'):
            headcount=entity['text']
    if( headcount):
        responder.slots['headcount'] = headcount
    else:
        responder.slots['headcount'] = "NA"
    responder.reply("Number of people confirmed : {headcount}")